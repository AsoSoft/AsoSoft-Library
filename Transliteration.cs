...
namespace AsoSoftLibrary
{
    public static partial class AsoSoft
    {
	   ...
	   public static string Phonemes2IPA(string text)
	   {
		  ...
		  foreach (var item in replaceFiles.Phoneme2IPA.Split('\n'))
			 text = text.Replace(item.Split('\t')[0], item.Split('\t')[1]);
		  return text;
	   }
	   ...
	   public static string Phonemes2ASCII(string text)
	   {
		  ...
		  foreach (var item in replaceFiles.Phoneme2Ascii.Split('\n'))
			 text = Regex.Replace(text, item.Split('\t')[0], item.Split('\t')[1] + "▪");
		  return text;
	   }
    }
}
